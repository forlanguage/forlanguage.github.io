(() => {
  "use strict";
  const ROOT="/aptis/";
  const fail=(message)=>{document.body.innerHTML=`<p style="padding:30px">${message}</p>`;};
  function loadScript(src){return new Promise((resolve,reject)=>{const script=document.createElement("script");script.src=src;script.onload=resolve;script.onerror=()=>reject(new Error(`Không tải được ${src}`));document.head.appendChild(script);});}
  ...
  (async()=>{
    ...
    source=source.replaceAll('loadJson("data/','loadJson("/aptis/data/').replace("1R8Gg8of2uZnp7xYPIIWFfH5CpADR5_cYQYSgT8zKxg0","1Bjrp_IgqbhYdQmxF3Augs8rkHn-c4tZed-jFVqObNLI").replace("reading.tests.length!==10 || reading.items.length!==290 || readingMetadata.item_count!==290","reading.tests.length!==24 || reading.items.length!==696 || readingMetadata.item_count!==696").replace("Không tải được ngân hàng Aptis v3.","Không tải được ngân hàng Aptis v6.");
